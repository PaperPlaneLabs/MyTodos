...
#[tauri::command]
pub fn create_section(db: State<DbConnection>, project_id: i64, name: String) -> Result<Section> {
    let conn = db.lock();
...
#[tauri::command]
pub fn update_section(db: State<DbConnection>, id: i64, name: String) -> Result<()> {
    let conn = db.lock();

    let rows = conn.execute("UPDATE sections SET name = ? WHERE id = ?", (&name, id))?;
